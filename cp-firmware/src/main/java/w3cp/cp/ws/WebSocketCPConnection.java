...
public class WebSocketCPConnection implements CPConnection {
  ...
  private final Event<ConnectionOpenedEvent> connectionOpenedEvent;
  private final AtomicReference<WebSocket> webSocketRef = new AtomicReference<>();
  private final AtomicReference<WebSocket> verifiedWebSocketRef = new AtomicReference<>();
  ...
  @Inject
  public WebSocketCPConnection(BackendConfig backendConfig, Vertx vertx, Event<ConnectionOpenedEvent> connectionOpenedEvent) {
    this.backendUri = backendConfig.uri();   // e.g. wss://w3cp.web3-energy.com/w3cp
    this.vertx = vertx;
    this.connectionOpenedEvent = connectionOpenedEvent;
  }
  ...
  public Uni<Void> connect() {
    ...
    return client.webSocketAbs(
        ...
        .onItem().invoke(ws -> {
          ...
          connectionOpenedEvent.fireAsync(new ConnectionOpenedEvent());
  ...
  public boolean isVerified() {
    ...
    WebSocket verified = verifiedWebSocketRef.get();
    return current != null && current == verified && !current.isClosed();
  }
  ...
  public void markVerified() {
    ...
    if (current != null && !current.isClosed()) {
      verifiedWebSocketRef.set(current);
      log.info("✅ Connection marked as verified");
    }
  }
}
