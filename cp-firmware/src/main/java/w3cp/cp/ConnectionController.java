...
public class ConnectionController {
    ...
    try {
      if (autoReconnect && !connection.isConnected()) {
        ...
        connection.connect().subscribe().with(
            unused -> {},
            failure -> {
              if (!isShutdownError(failure)) {
                log.error("Reconnect failed.", failure);
              }
            }
        );
      }
      ...
      if (!isShutdownError(e)) {
        log.error("Error in checkAndReconnect", e);
      }
    }
  }
  ...
  private boolean isShutdownError(Throwable t) {
    ...
    return msg != null && (msg.contains("Error Occurred After Shutdown") || 
